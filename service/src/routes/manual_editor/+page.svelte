  ...
  {:else}
    ...
    <div class="flex justify-between">
      ...
      <div class="p-2 flex flex-col shadow-xl rounded-lg">
        <div class="flex">
          <button class="bg-red-400 hover:bg-red-300 text-sm mr-2 px-1 whitespace-nowrap text-white rounded-xl"
                  on:click={() => {set_file = null; time_now = 0;}}>
            Удалить проект
          </button>
        ...
        {#if point_voc[time_now - (time_now % 1)]}
          <div class="border mt-2 p-1 rounded-lg flex flex-col">
            ...
            {:else}
              {#if !recording_state}
                <button class="btn-blue mt-2 mx-auto" on:click={() => start_recording(time_now)}>Создание аудио
                  дорожки
                </button>
              {:else}
      ...
      <div class="flex px-4 mt-1 mb-4 text-lg bg-blue-100">
        <div class="flex mx-4 w-full relative bg-green-100 ">
          {#each point_list as point}
            <div class="w-2 h-2 bg-red-600 absolute cursor-pointer rounded-full" on:click={() => time_now = point}
                 style={"left: calc(" + ((point / duration) * 100) + "% - 8px);"}></div>
          {/each}
        </div>
      </div>
